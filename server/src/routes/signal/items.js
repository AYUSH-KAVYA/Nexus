...
// Update extracted item
  ...
  // AS-06: Emit domain events for cross-app architecture demonstration
  try {
    const projRes = await client.query('SELECT organization_id FROM projects WHERE id = $1', [project_id]);
    const organizationId = projRes.rows[0]?.organization_id;

    if (organizationId) {
      // 1. Emit CONVERSATION_ITEM_CONFIRMED from nexus_signal
      await client.query(
        `INSERT INTO domain_events (organization_id, event_type, source_app, payload)
         VALUES ($1, 'CONVERSATION_ITEM_CONFIRMED', 'nexus_signal', $2)`,
        [
          organizationId,
          JSON.stringify({
            item_id: item.id,
      ...
      // 2. Cross-app call: Invoke Nexus impact engine and log IMPACT_ENGINE_TRIGGERED
      if (resolvedTaskId) {
        const impact = await getImpact(resolvedTaskId, project_id, { dryRun: false });
        await client.query(
          `INSERT INTO domain_events (organization_id, event_type, source_app, payload)
           VALUES ($1, 'IMPACT_ENGINE_TRIGGERED', 'nexus', $2)`,
          [
            organizationId,
            JSON.stringify({
              ...
              blast_radius_level: impact.blastRadiusLevel,
              affected_tasks_count: impact.affectedTasks ? impact.affectedTasks.length : 0,
              approval_gates_reopened: impact.approvalGatesReopened || 0
            })
          ]
        );
      }
    ...
    console.error('Error emitting domain event in confirmSingleItem:', eventErr);
  }
